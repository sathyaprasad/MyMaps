...
public class MapControlWidget {
	...
	public static double rotateMap(final MapViewerActivity activity,
			final double currentAzumith, final boolean sendMessage) {
		double tmp = 0;
		Log.i("bluetooth", "get raw degree: " + Double.toString(currentAzumith));
		...
		Log.i("bluetooth", "get degree: " + Double.toString(degree));
		new Handler().post(new Runnable() {
			public void run() {
	...
	public static void queryMapView(MapViewerActivity activity, double x,
			double y, boolean sendMessage) {
		MapControlUIBuilder.pointQueried = new Point(x, y);
		new CalloutQuery(activity).queryPopup((float) x, (float) y);
		if (sendMessage) {
			activity.bluetoothFragment.messageBuilder(
					BluetoothFragment.SINGLE_TAP, new double[] { x, y });
		}
	}
		...
		if (layerSwitch == null) {
			...
			for (int i = 0; i < layerControl.getChildCount(); i++) {
				...
				if (v.getTag() != null
					...
					layerSwitch = (CheckBox) ((LinearLayout) ((LinearLayout) v)
							.getChildAt(0)).getChildAt(0);
				}

				// if (v.getTag() != null) {
				// try {
				// CheckBox tmp = (CheckBox) v.getTag();
				// if (Integer.parseInt(tmp.getTag().toString()) == index) {
				// layerSwitch = tmp;
				// break;
				// }
